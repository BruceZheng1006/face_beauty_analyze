    ...
    try:
        ...
        if result.face_landmarks:
            for face_idx, landmarks in enumerate(result.face_landmarks):
                ...
                mandible_angle = get_angle(landmarks[361], landmarks[397], landmarks[152], img_w, img_h)  # 下额角度数
                ...
                print(f"fWHR(面部宽高比): {zygomatic_width / upface_height:.2f}")
                ...
                print(f"眼睛高度: {eye_height:.2f} mm")
                print(f"眼睛宽度: {eye_width:.2f} mm")
                print(f"内眦角度数: {inner_eye_angle:.2f} °")

                # 7. 鼻子
                print(f"\n● 鼻子")
                print(f"鼻翼宽度: {get_distance(landmarks[129], landmarks[358], img_w, img_h) * factor:.2f} mm")
                ...
                # 嘴角点(291)与唇中内缘点(13)
                mouth_angle = get_twopoint_angle(landmarks[291], landmarks[308], img_w, img_h) - 90
                print(f"嘴唇高度: {lip_h:.2f} mm")
                print(f"嘴唇宽度: {lip_w:.2f} mm")
...
if __name__ == "__main__":
    ...
    # 2. 模型初始化
    # model_path = "D:\\Bias\\8.3_dream\\face_beauty_analyze\\scripts\\models\\face_landmarker.task"
    # 优先推荐：正斜杠相对路径（Windows/Linux/Mac都支持，无需转义）
